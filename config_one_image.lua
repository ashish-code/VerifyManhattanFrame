...
function config.parse(arg)
    ...
    -- testing
    cmd:option('-test_model', './model/sync_physic_nyufinetune.t7', 'model used for testing')
    cmd:option('-result_path', 'temp.jpg', 'path to save result')
    cmd:option('-max_count', 1, 'max number of data to test')
    cmd:option('-input_file', './image/0000_color.png', 'input image file')

    -- data loader
    cmd:option('-train_file', '', 'train file, compulsory');
    cmd:option('-test_file',  'temp.txt', 'test file, compulsory');
